            ...
            success: function(data) {
                ...
                if (Array.isArray(products) && products.length > 0) {
                    $.each(products, function(index, product) {
                        // Pastikan menggunakan data produk yang benar
                        const productId = product.id;
                        const storeId = product.store_id;
                        const categoryId = product.category_id;
                        const productSlug = product.slug; // Gunakan slug dari data produk
                        
                        // ... kode rating dan tampilan lainnya ...
                        
                        var productHtml = `
                            <div class="product-box ${isTokoOnline ? '' : 'toko-tutup'}">
                                <span hidden>${productId}</span>
                                <span hidden>${storeId}</span>
                                <span hidden>${productSlug}</span>
                                <img alt="${product.name}" src="${product.photo}">
                                <strong>${product.name}</strong>
