                    ...
                    success: function(data) {
                        ...
                        if (Array.isArray(products) && products.length > 0) {
                            $.each(products, function(index, product) {
                                ...
                                // Periksa apakah toko online
                                const tokoOnline = product.toko.is_online ? '' : 'toko-tutup'; // Periksa apakah toko buka
                                const tokoStatus = product.toko.is_online ? '' : '<span class="text-red-500">(Toko Tutup)</span>';
    
                                var productHtml = `
                                    <div class="product-box ${tokoOnline}">
                                        <img alt="${product.name}" src="${product.photo}">
                                        <strong>${product.name}</strong>
                                        <span class="quantity">Kategori: ${product.category ? product.category.name : 'Unknown'}</span>
                                        <span class="quantity">Toko: ${product.toko ? product.toko.nama_toko : 'Unknown'} ${tokoStatus}</span>
                                        <div class="flex">
                                            ${starsHtml}
