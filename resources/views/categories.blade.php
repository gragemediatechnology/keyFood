                ...
                success: function(data) {
                    ...
                    if (Array.isArray(products) && products.length > 0) {
                        $.each(products, function(index, product) {
                            ...
                            // Cek status toko
                            var tokoStatus = product.toko && product.toko.isOpen ? '' : 'toko-tutup';
                            var tokoLabel = product.toko && !product.toko.isOpen ? ' (Toko Tutup)' : '';

                            var productHtml = `
                                <div class="product-box ${tokoStatus}">
                                    <img alt="${product.name}" src="${product.photo}">
                                    <strong>${product.name}</strong>
                                    <span class="quantity">Kategori: ${product.category ? product.category.name : 'Unknown'}</span>
                                    <span class="quantity">Toko: ${product.toko ? product.toko.nama_toko : 'Unknown'} ${tokoLabel}</span>
                                    <div class="flex">
                                        ${starsHtml}
