...
<main class="h-screen pb-16 overflow-y-auto">
    <div class="container grid px-6 mx-auto py-4 mb-8">
        ...
        {{-- ini cards --}}
        <div class="container-profile" id="role-requests-container">
            @forelse ($roleRequests->sortByDesc('id') as $request)
                ...
                <div class="card-profile">
                    ...
                    <div class="btn-container">
                        ...
                        {{-- <form action="{{ route('role-request.approve', $request->id) }}" method="POST" --}} <form
                            action="/role-request/approve/{{ $request->user_id }}" method="POST" style="display: inline;">
                            @csrf
                            <button
                        ...
                        {{-- <form action="{{ route('role-request.cancel', $request->id) }}" method="POST" --}} <form
                            action="/role-request/cancel/{{ $request->user_id }}" method="POST" style="display: inline;">
                            @csrf
                            <button
                        ...
                        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            @forelse ($roleRequests as $request)
                                <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center text-sm">
                                            {{-- <form action="{{ route('role-request.approve', $request->id) }}" --}} <form
                                                action="/role-request/approve/{{ $request->user_id }}" method="POST"
                                            ...
                                            {{-- <form action="{{ route('role-request.cancel', $request->id) }}" --}} <form
                                                action="/role-request/cancel/{{ $request->user_id }}" method="POST"
                                                method="POST" style="display: inline;">
                                                @csrf
                                                <button
                                                    class="cancel flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
...
$(document).ready(function() {
    let loading = false;
    let nextPageUrl = "{{ $roleRequests->nextPageUrl() }}";  // Get the URL for the next page
            ...
            if (nextPageUrl) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#role-requests-container').append(response.html);

                        // Update the next page URL
                        nextPageUrl = response.nextPage ? response.nextPage : null;
                    },
                    error: function() {
                        loading = false;
                        $('#loading').hide();  // Hide loading spinner
